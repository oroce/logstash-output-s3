...
class LogStash::Outputs::S3 < LogStash::Outputs::Base
  ...
  config :object_name, :validate => :string, :default => ""
  ...
  # if you lose connection with s3, bad control implementation.
    ...
    object_name = file_basename.clone;
    unless @object_name.empty?
      @logger.debug "sprinfing object name ("+@object_name+")"
      object_name = sprintf(@object_name)
    end
    ...
    @logger.debug "S3: ready to write "+object_name+"("+file_basename+") in bucket "+@bucket+", Fire in the hole!"

    object = bucket.objects[object_name]
    if @gzip
      object.write(gzip(file_data), :acl => @canned_acl, :content_encoding => "gzip" )
    else
